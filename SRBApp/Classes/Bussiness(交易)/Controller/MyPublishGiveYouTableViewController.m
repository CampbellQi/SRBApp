//
//  MyPublishGiveYouTableViewController.m
//  SRBApp
//
...
@interface MyPublishGiveYouTableViewController ()<UITableViewDataSource, UITableViewDelegate>
{
    MBProgressHUD * hud;
}
@end

@implementation MyPublishGiveYouTableViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString * timeStr = [CompareCurrentTime compareCurrentTime:[_dataArray[indexPath.row]updatetimeLong]];
    cell.nameLabel.text = timeStr;
    if ([_dealType isEqualToString:@"1"]) {
    ...
    if ([[_dataArray[indexPath.row]freeShipment] isEqualToString:@"1"]) {
        ...
        cell.postLabel.frame = CGRectMake(cell.priceLabel.frame.origin.x + cell.priceLabel.frame.size.width + 7, cell.priceLabel.frame.origin.y + 2, 30, 16);
    }
    else
    {
...
- (void)urlRequestPost
{
    ...
    dic = [self parametersForDic:@"getPostListByUser" parameters:@{ACCOUNT_PASSWORD,@"user":ACCOUNT_SELF, @"type":@"0", @"dealType":@"1",@"status":@"1", @"start":@"0", @"count":[NSString stringWithFormat:@"%d",count]}];
...
- (void)footerRefresh
{
    ...
    NSDictionary * dic;
    dic = [self parametersForDic:@"getPostListByUser" parameters:@{ACCOUNT_PASSWORD,@"user":ACCOUNT_SELF, @"type":@"0", @"dealType":@"1",@"status":@"1", @"start":[NSString stringWithFormat:@"%d",page], @"count":[NSString stringWithFormat:@"%d",NumOfItemsForZuji]}];
