...
@interface RunViewController ()<TuSDKPFCameraDelegate, TuSDKPFEditTurnAndCutDelegate, TuSDKFilterManagerDelegate>
{
    UILabel * positionLB;
...
//    [self showHubWithStatus:LSQString(@"lsq_initing", @"正在初始化")];
    ...
    UIView * view4 = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 180)];
    [view4 addSubview:[self write]];
    [_scrollView addSubview:view4];
    
    UIView * view5 = [[UIView alloc]initWithFrame:CGRectMake(0, view4.frame.origin.y + view4.frame.size.height + 30, SCREEN_WIDTH, 80)];
    [view5 addSubview:[self position]];
    [_scrollView addSubview:view5];
    
    //选择标签
    UIView * signView = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(view5.frame) + 5, SCREEN_WIDTH, 40)];
    signView.backgroundColor = [GetColor16 hexStringToColor:@"#ffffff"];
    [_scrollView addSubview:signView];
    
    UITapGestureRecognizer * signTap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(signTap)];
    [signView addGestureRecognizer:signTap];
    
    UILabel * signLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, 11, 60, 18)];
    signLabel.font = SIZE_FOR_14;
    signLabel.textColor = [GetColor16 hexStringToColor:@"#434343"];
    signLabel.text = @"标签";
    [signView addSubview:signLabel];
    
    //显示标签
    UILabel * showSignLabel = [[UILabel alloc]initWithFrame:CGRectMake(SCREEN_WIDTH - 15 - 10 - 100, 11, 100, 18)];
    showSignLabel.font = SIZE_FOR_14;
    self.showSignLabel = showSignLabel;
    showSignLabel.textColor = [GetColor16 hexStringToColor:@"#959595"];
    [signView addSubview:showSignLabel];
    
    UIImageView * detailImg = [[UIImageView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH - 15 - 5, 15, 5, 10)];
    detailImg.image = [UIImage imageNamed:@"friendnew_jt"];
    [signView addSubview:detailImg];
    
    _scrollView.contentSize = CGSizeMake(SCREEN_WIDTH, view5.frame.origin.y + view5.frame.size.height + 10 + CGRectGetMaxY(signView.frame));
    //开启定位
...
//    
    publishLater = [[PublishLaterView alloc]initWithFrame:CGRectMake(0,0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    ...
    [publishLater.shareButton addTarget:self action:@selector(shareButton:) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)upimagefengzhuang:(TuSDKResult * )result
{
    ...
    if (imageNumber == 0) {
        ...
        [self uploadPictureWithImageData:image];
...
- (void)signTap
{
    ...
    createSignVC.signStr = self.signStr;
    createSignVC.completeBlock = ^(NSString *signStr) {
        self.showSignLabel.text = signStr;
        self.signStr = signStr;
    };
    //createSignVC.signArrs = self.signArray;
...
- (UIView *)write
{
    ...
    detailTV = [[UITextView alloc]initWithFrame:CGRectMake(15, 20 , SCREEN_WIDTH - 30, 80)];
...
//    detailTV.inputAccessoryView = finishiView1;
    ...
    UIView * view1 = [[UIView alloc]initWithFrame:CGRectMake(0, 100, SCREEN_WIDTH, SCREEN_HEIGHT / 667 * 105)];
    [view1 addSubview: [self addPhoto]];
    [view addSubview:view1];
    
//    numLabel = [[UILabel alloc]initWithFrame:CGRectMake(SCREEN_WIDTH - 80, 0, 65, 14)];
//    numLabel.textAlignment = NSTextAlignmentRight;
//    numLabel.textColor = [GetColor16 hexStringToColor:@"#c9c9c9"];
//    if (detailTV.text.length == 0) {
//        numLabel.text = @"0/140";
//    }else{
//        numLabel.text = [NSString stringWithFormat:@"%lu/48" , (unsigned long)detailTV.text.length];
//    }
//    [view1 addSubview:numLabel];
    
    
    
    numLabel = [[UILabel alloc]initWithFrame:CGRectMake(SCREEN_WIDTH - 100, detailTV.frame.size.height + detailTV.frame.origin.y + 5, 80, 15)];
...
- (void)backBtn:(UIButton *)sender
{
    ...
    if (detailTV.text.length == 0 && _imageArr.count == 0) {
...
//        [_bigImageArr addObject:image];
        ...
        [self uploadPictureWithImageData:image1];
        uploadnum = 1;
    }
...
-(void)uploadPictureWithImageData:(UIImage *)image{
...
- (void)post
{
    ...
    if (detailStr == nil || detailStr.length == 0 || [detailStr isEqualToString:@""]) {
        detailStr = @"0";
    }
    
    if (_imageArr.count == 0 && ([detailStr isEqualToString:@"0"])){
        [AutoDismissAlert autoDismissAlert:@"请填写分享内容,或上传图片"];
        [detailTV becomeFirstResponder];
        publishNum = 0;
        return;
    }
    if (detailTV.text.length > 140) {
        [AutoDismissAlert autoDismissAlert:@"足迹发布不能多于140字"];
        [detailTV becomeFirstResponder];
        publishNum = 0;
        return;
    }
    ...
    if ([positionsign isEqualToString:@"1"]) {
        NSDictionary * dic = [self parametersForDic:@"accountCreatePosition" parameters:@{ACCOUNT_PASSWORD, @"title":position, @"xyz":xyz ? xyz : @"0", @"url":string, @"content":detailStr, @"uuid":@"0",@"tags":self.signStr}];
        //发送post请求
...
//                [self.circleVC urlRequestPost];
//                [self.relationVC urlRequestPost];
                [HUD removeFromSuperview];
//                if (self.relationBlock != nil) {
//                    self.relationBlock(@"1");
//                }
                
               [self.navigationController dismissViewController];
                //                [publishLater setHidden:NO];
        ...
        NSDictionary * dic = [self parametersForDic:@"accountCreatePosition" parameters:@{ACCOUNT_PASSWORD, @"title":@"0", @"xyz":@"0", @"url":string, @"content":detailStr, @"uuid":@"0",@"tags":self.signStr}];
        //发送post请求
        [URLRequest postRequestssWith:iOS_POST_URL parameters:dic andblock:^(NSDictionary *dic) {
