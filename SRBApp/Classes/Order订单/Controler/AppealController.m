...
@interface AppealController ()<ODMCombinationPickerViewControllerDelegate>
{
...
@implementation AppealController
...
- (void)viewDidLoad {
    ...
    [self.reasonSuperView addTapAction:@selector(showPickerView) forTarget:self];
...
-(void)showPickerView{
    if (!_pickerView) {
        NSString *s = @"退款申请不合理,对方拒绝沟通,始终无法达成意见一致";
        ...
        pickerView.confirmBlock = ^(NSString *selectedItem) {
            self.reasonTF.text = selectedItem;
        };
    }
...
-(void)submitClicked {
    ...
    NSString * reason = [self.reasonTF.text trim];
    
    if (reason.length == 0) {
        [AutoDismissAlert autoDismissAlert:@"请填申诉原因"];
        return;
    }
    if (content.length == 0) {
        [AutoDismissAlert autoDismissAlert:@"请填申诉内容"];
        return;
    }
...
- (void)uploadAllRequest
{
    NSString * content = [self.contentTV.text trim];
    //拼接post参数
    NSDictionary * dic = [self parametersForDic:@"addTaskReport" parameters:@{ACCOUNT_PASSWORD, @"title": self.reasonTF.text, @"id":_spOrderID, @"msg": content, @"qqNo": @"", @"telNo":_mobileTF.text, @"email":@"",@"photos":_imagesUrlStr}];
    //发送post请求
