...
@interface MyEvaluateViewController ()<UITextViewDelegate>
...
//    NSDictionary * dic = [self parametersForDic:@"accountSetOrderComment" parameters:@{@"account":name,@"password":pass,@"orderId":self.toSellerModel.orderId,@"itemId":self.toSellerModel.itemId,@"grade":@"",@"content":self.textView.text}];
...
- (void)customInit
{
    ...
    UIView * kaopuManyView = [[UIView alloc]initWithFrame:CGRectMake(kaopuLable.frame.size.width + kaopuLable.frame.origin.x, kaopuLable.frame.origin.y, 70, 10)];
...
- (void)makeValueToControl
{
    ...
    //靠谱,忽悠  长度
    self.huyouBgView.backgroundColor = [GetColor16 hexStringToColor:@"#f6f6f6"];
    self.kaopuBgView.frame = CGRectMake(0, 0, kaopu, 10);
    self.huyouBgView.frame = CGRectMake(0, 0, huyou, 10);
}

#pragma mark - 提交按钮
- (void)commentSubmit:(UIButton *)submit
{
    ...
    NSDictionary * dic = [self parametersForDic:@"accountSetOrderComment" parameters:@{@"account":name,@"password":pass,@"orderId":self.toSellerModel.orderId,@"itemId":self.toSellerModel.itemId,@"grade":orderGrade,@"content":sayStr}];
    [URLRequest postRequestWith:iOS_POST_URL parameters:dic andblock:^(NSDictionary *dic) {
