...
- (void)viewDidLoad {
    ...
    _sendBtn.layer.cornerRadius = CGRectGetHeight(_sendBtn.frame)*0.5;
...
-(void)commentRequest:(NSString *)content {
    ...
    NSString *modelID = @"";
    if (self.sourceModel) {
        modelID = self.sourceModel.model_id;
    }else if (_modelID) {
        modelID = _modelID;
    }
    NSDictionary * param = [self parametersForDic:@"accountCommentPost" parameters:@{ACCOUNT_PASSWORD,@"markId":markId, @"id":modelID, @"content":content}];
...
- (void)loadNewDataListRequest
{
    ...
    NSDictionary * param = [self parametersForDic:@"getPostCommentsByRelation" parameters:@{ACCOUNT_PASSWORD,@"isFriended":@"0",@"id":modelID, @"start":@"0", @"count":[NSString stringWithFormat:@"%d",count]}];
...
- (void)loadMoreDataListRequest
{
    ...
    page += NumOfItems;
    NSDictionary * param = [self parametersForDic:@"getPostCommentsByRelation" parameters:@{ACCOUNT_PASSWORD, @"isFriended":@"0", @"id":modelID, @"start":[NSString stringWithFormat:@"%d",page], @"count":[NSString stringWithFormat:@"%d",count]}];
    [URLRequest postRequestssWith:iOS_POST_URL parameters:param andblock:^(NSDictionary *dic) {
        NSString * result = [dic objectForKey:@"result"];
        if ([result isEqualToString:@"0"]) {
            NSArray * temparrs = [[dic objectForKey:@"data"] objectForKey:@"list"];
...
-(void)deleteCommentRequest {
    ...
    [URLRequest postRequestWith:iOS_POST_URL parameters:dic andblock:^(NSDictionary *dic) {
        ...
        if (result == 0) {
            ...
            _isComment = YES;
            if (self.sourceModel) {
                int count = [self.sourceModel.consultCount intValue] - 1;
                self.sourceModel.consultCount = [NSString stringWithFormat:@"%d", (count - 1 >= 0) ? count : 0];
            }
