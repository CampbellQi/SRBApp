...
@interface MarkTopicListController ()<ISSShareViewDelegate>
{
...
- (void)shareBtn:(id)sender
{
    ...
    NSString *content = [NSString stringWithFormat:@"%@ %@",_markDataDict[@"name"], _shareDict[@"shortUrl"]];
    
    if (content.length > 140) {
        content = [content substringToIndex:140];
    }
    [ShareViewController shareToThirdPlatformWithUIViewController:self Title:_markDataDict[@"name"] SecondTitle:nil Content:content ImageUrl:_markDataDict[@"pic"] SencondImgUrl:nil Btn:sender ShareUrl:_markDataDict[@"shareUrl"]];
...
- (void)shareRequest
{
    NSDictionary * dic = [self parametersForDic:@"getShortUrl" parameters:@{ACCOUNT_PASSWORD,@"url":_markDataDict[@"shareUrl"]}];
    [URLRequest postRequestWith:iOS_POST_URL parameters:dic andblock:^(NSDictionary *dic) {
        NSString *result = [dic objectForKey:@"result"];
        if ([result isEqualToString:@"0"]) {
            _shareDict = [NSMutableDictionary dictionaryWithDictionary:[dic objectForKey:@"data"]];
...
- (void)loadMoreDataListRequest
{
    ...
    NSDictionary * param = [self parametersForDic:@"getPostListBySearchKeyword" parameters:@{ACCOUNT_PASSWORD,@"type":@"1042", @"tags": _markDataDict[@"name"], @"start":[NSString stringWithFormat:@"%d", _page], @"count":[NSString stringWithFormat:@"%d",_count], @"order": @"postid"}];
    [URLRequest postRequestssWith:iOS_POST_URL parameters:param andblock:^(NSDictionary *dic) {
        _topicCount = @"0";
        NSString * result = [dic objectForKey:@"result"];
        if ([result isEqualToString:@"0"]) {
