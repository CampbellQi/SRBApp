...
- (UIImageView *)tempImg
{
    if (!_tempImg) {
        //动画
        UIImageView * tempImage = [[UIImageView alloc]initWithFrame:CGRectMake((SCREEN_WIDTH - 100) /2 - 30, 10, 24, 40)];
        [self addSubview:_tempImg = tempImage];
        ...
        [tempImage setAnimationImages:arr];
        [tempImage setAnimationDuration:0.2];
    }
    return _tempImg;
}
...
- (UIImageView *)arrowImage
{
    if (!_arrowImage) {
        ...
        UIImageView *arrowImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:MJRefreshSrcName(@"333")]];
        arrowImage.frame = CGRectMake((SCREEN_WIDTH - 100) /2 - 30, 10, 24, 40);
...
- (void)layoutSubviews
{
    ...
    self.tempImg.frame = CGRectMake((self.mj_width - 100) /2 - 30, 10, 24, 40);
    
    // 1.箭头
    self.arrowImage.frame = CGRectMake((self.mj_width - 100) /2 - 30, 10, 24, 40);
    CGPoint tempCenter = self.arrowImage.center;
    tempCenter.y = self.mj_height * 0.5;
    self.arrowImage.center = tempCenter;
...
- (void)settingFlashImg
{
    switch (self.state) {
        case MJRefreshStateNormal:
        {
            [self.tempImg stopAnimating];
            break;
        }
        case MJRefreshStatePulling:
        {
